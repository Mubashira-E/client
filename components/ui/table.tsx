...
function TableHeader({ className, ...props }: React.ComponentProps<"thead">) {
  return (
    <thead
      ...
      className={cn("[&_tr]:border-b bg-teal-50/30 rounded-t-xl", className)}
      {...props}
    />
  );
}
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "hover:bg-teal-50/20 data-[state=selected]:bg-teal-50/40 border-b transition-colors text-foreground",
        className,
      )}
      {...props}
    />
