...
import { importTreatmentFile } from "@/lib/visit-api";
...
export function ExcelUploadContainer() {
  ...
  const [pendingUpload, setPendingUpload] = useState<{ file: File; rowsWithoutValues: number; validRowsCount?: number; missingColumns?: string } | null>(null);
  ...
  const uploadFile = async (file: File, continueOnInvalidRows: boolean = false) => {
    try {
      const data = await importTreatmentFile(file, continueOnInvalidRows);
      ...
      if (statusCode === 400 && (errorData?.rowsWithoutValues || errorData?.detail?.includes("don't have") || errorData?.detail?.includes("don’t have"))) {
        ...
        if (errorData?.detail) {
          const invalidMatch = errorData.detail.match(/(\d+)\s+rows?\s+don['\u2019]t have\s+(\S.*?)(?:\.|\?|$)/i);
          if (invalidMatch) {
        ...
        setPendingUpload(prev =>
          prev ? { ...prev, rowsWithoutValues: invalidRowsCount, validRowsCount, missingColumns } : null,
        );
  ...
  return (
    <div className="flex flex-col">
      <section className="relative bg-white p-4  rounded-md">
        <Tabs defaultValue="upload" value={selectedTab} onValueChange={setSelectedTab} className="relative">
