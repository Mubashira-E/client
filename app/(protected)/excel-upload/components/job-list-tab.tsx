...
import type { UploadJob } from "./excel-upload-container";
import Lottie from "lottie-react";
import { CheckCircle, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Clock, Loader2, RefreshCw, XCircle } from "lucide-react";
...
export function JobListTab({ jobs, currentPage, totalPages, totalItems, isLoading, onPageChange, onRefresh, lastRefreshTime }: JobListTabProps) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-center mt-12">
        ...
        <div className="flex flex-col items-center justify-center space-y-2">
          ...
          <p className="text-muted-foreground">Get started by uploading your first Excel file to track its progress here</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <section className="space-y-4 mt-6">
        ...
        {jobs.map((job) => {
          ...
          return (
            <section
              ...
              className="bg-white border rounded-md p-4 cursor-pointer hover:bg-gray-100 transition-colors hover:border-primary/50"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 flex-1">
                  <Image src={ExcelLogo} alt="Excel Logo" width={48} height={48} />
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold truncate text-primary">{job.fileName}</p>
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      {job.uploadedAt && (
                        <span>
                          ...
                          at
                          {" "}
                          {job.uploadedAt.toLocaleTimeString()}
                        </span>
                      )}
                      {job.totalRows > 0 && (
                        <>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2">
                    <Badge variant={config.variant} className={normalizedStatus === "completedwitherrors" ? "text-white" : ""}>{config.label}</Badge>
                  </div>
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      e.preventDefault();
                      handleViewJobDetails(job.jobId, normalizedStatus);
                    }}
                  >
      ...
      <AlertDialog open={showInProgressModal} onOpenChange={setShowInProgressModal}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <div className="flex items-center gap-3 mb-2">
              ...
              />
              <AlertDialogTitle className="text-xl">Job Still Processing</AlertDialogTitle>
            </div>
            <AlertDialogDescription className="text-base leading-relaxed">
              This upload is currently being processed. Please wait a moment and check back shortly to view the details.
